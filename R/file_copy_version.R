...
#' @export
file_copy_version <- function(path, ref = "HEAD", name = NULL, dir = NULL, full_name = NULL, recurse = TRUE, repo = ".") {
  path(vapply(path, \(x) file_copy_version_single(x, ref, repo, name, dir, full_name, recurse, repo), character(1)))
}

#' @inheritParams commits_between
#' @export
#' @rdname file_copy_version
file_copy_versions <- function(path, from = "HEAD", to = NULL, name = NULL, dir = NULL, file_filter = NULL, repo = ".") {
  commits <- commits_between(from = from, to = to, repo = repo)
  versions <- lapply(commits, \(x) file_copy_version(path, x, name, dir, fir_full, recurse, repo))
  names(versions) <- vapply(commits, sha, character(1))
  versions
}

file_copy_version_single <- function(path, ref, name, dir, full_name, recurse, repo) {
  commit <- as_commit(ref, repo)

  if (is.null(full_name)) full_name <- is.null(name) && is.null(dir)
  if (is.null(name)) name <- path_file(path)
  if (is.null(dir)) dir <- path(sys_tempdir(), sha(commit))
  ...
  if (is_tree(obj)) {
    dir_create(out_path)
    return(file_copy_recursive(obj, dir = out_path, recurse = recurse, repo = repo))
  } else if (is_blob(obj)) {
    blob_to_file(obj, out_path, obj$mode)
  } else {
...
file_copy_recursive <- function(tree, dir, recurse, repo) {
  ...
  if (recurse > 0) {
    for (i in seq_along(obj)) {
      ...
      if (inherits(obj_i, "git_tree")) {
        dir_create(path_i)
        file_copy_recursive(obj_i, name = name_i, dir = path_i, recurse = recurse - 1)
